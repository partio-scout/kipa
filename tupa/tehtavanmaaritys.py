...
peruskaava = "max(interpoloi(" + suoritus + "," + maksimisuoritus
peruskaava += ",jaettavat," + nollasuoritus + "))"


"""
This file is made with no explainable logic. It has only been made to work.
Most propably the best way to improveis simply rewriting. Anyway the task is
not simple. Strongly recommend of using the already defined database syntax.
See developement documentation for the actual syntax definition.
"""
...
def aika_numeroksi(s):
    ...
    if haku:
        return str(int(haku.group(1)) * 60 * 60 + int(haku.group(2)) * 60 +
                   int(haku.group(3)))
    else:
        return s
...
def syotteen_tyyppi_field(posti, data, prefix, syote_id, tyyppi):
    ...
    # formin taytto
    if data['tyyppi'] == tyyppi or (not posti and (tyyppi == "vk" or tyyppi == "pk")):  # noqa
        maarite_index = 0
        maaritteet = []
        for k, v in data['maaritteet'].items():
            maaritteet.append((v['nimi'], k))
...
def saveField(posti, data, field_name, data_path, nimi, field,
              prefix="", muunnos=None):
    id = prefix + "_" + field_name
    if posti and id in posti.keys():
        value = posti[id]
...
def loadField(state, data, field_name,
              data_path, nimi, field, prefix=None, muunnos=None):
    if prefix:
        id = prefix + "_" + field_name
    else:
        id = field_name
...
def syotteen_kuvaus_field(posti, data, prefix, syote_id, tyyppi):
    ...
    if (posti and prefix in posti.keys() and posti[prefix] == tyyppi or
        not posti and (data['tyyppi'] == tyyppi or
                       tyyppi == 'pk' or tyyppi == 'vk')):
        # Formin data:
        ...
        if not formi:
            ...
            formi = {field_name: {'id': id,
                                  ...
                                  'value': value,
                                  'errors': errors}}
            data['maaritteet']['#' + str(syote_id)] = {'nimi': nimi,
                                                       'kali_vihje': value}
    else:
        # valitsematon tyyppi
        if posti and id in posti.keys():
        ...
        formi = {field_name: {'id': id,
                              ...
                              'value': value,
                              'errors': errors}}
    if not formi[field_name]['value']:
        formi[field_name]['value'] = ""
    return formi


def poistaYlimaaraisetMaaritteet(posti, data, prefix, tyyppi,
                                 tarvittava_maara):
...
def lataa_parametrit(state, data, prefix, ot_tyyppi, muunnos=None):
    if ("tyyppi" in data.keys() and
       ...
       ot_tyyppi[1:] == "pk")):
        loadField(state, data, "kiintea", ['parametrit'],
                  'parhaan_kaava', 'arvo', prefix + ot_tyyppi)
        loadField(state, data, "jaettavat", ['parametrit'],
                  'jaettavat', 'arvo', prefix + ot_tyyppi)
        loadField(state, data, "parhaan_haku", ['parametrit'],
                  'parhaan_haku', 'arvo', prefix + ot_tyyppi)
        loadField(state, data, "nollan_kerroin", ['parametrit'],
                  'nollan_kerroin', 'arvo', prefix + ot_tyyppi)
        loadField(state, data, "nollan_kaava", ['parametrit'],
                  'nollan_kaava', 'arvo', prefix + ot_tyyppi)
        loadField(state, data, "oikea", ['parametrit'],
                  'oikea', 'arvo', prefix + ot_tyyppi)
        loadField(state, data, "kaava", ['parametrit'],
                  'vartion_kaava', 'arvo', prefix + ot_tyyppi)
        loadField(state, data, "arvio", ['parametrit'],
                  'arvio', 'arvo', prefix + ot_tyyppi)

        # Puhdaskaavan generointi muista tyypeistä
        if ot_tyyppi[1:] == "pk" and data['tyyppi'] != "pk":
            parametrit = {}
            loadField(parametrit, data, "parhaan_kaava", ['parametrit'],
                      'parhaan_kaava', 'arvo')
            loadField(parametrit, data, "jaettavat", ['parametrit'],
                      'jaettavat', 'arvo')
            loadField(parametrit, data, "parhaan_haku", ['parametrit'],
                      'parhaan_haku', 'arvo')
            loadField(parametrit, data, "nollan_kerroin", ['parametrit'],
                      'nollan_kerroin', 'arvo')
            loadField(parametrit, data, "nollan_kaava", ['parametrit'],
                      'nollan_kaava', 'arvo')
            loadField(parametrit, data, "oikea", ['parametrit'],
                      'oikea', 'arvo')
            loadField(parametrit, data, "vartion_kaava", ['parametrit'],
                      'vartion_kaava', 'arvo')
            loadField(parametrit, data, "arvio", ['parametrit'],
                      'arvio', 'arvo')
            loadField(parametrit, data, "tapa", ['parametrit'],
                      'tapa', 'arvo')
            kaava = prefix + ot_tyyppi + '_kaava'
            state[kaava] = luoOsatehtavanKaava(peruskaava, parametrit)
        try:
            if state[prefix + ot_tyyppi + "_parhaan_haku"] != "":
        ...
        try:
            if not state[prefix + ot_tyyppi + "_nollan_kerroin"] == "1":
                ...
                if muunnos:
                    value = muunnos(state[prefix + ot_tyyppi +
                                          '_nollan_kaava'])
        ...
        try:
            ...
            if kerroin != "1" and kerroin != "1.5" and kerroin != "0.5":
                state_key = prefix + ot_tyyppi + "_nollan_kerroin"
                state[prefix + ot_tyyppi + '_muu_kerroin'] = state[state_key]
        except:
            pass
        try:
            if state[prefix + ot_tyyppi + "_arvio"] == "":
                state[prefix + ot_tyyppi + "_oikea"] = ""
            else:
                if muunnos:
...
####################################
def maksimisuoritus(state, data, prefix, ot_tyyppi, formi,
                    validiointi=[[is_number], u"Syötä numeroita!"],
                    muunnos=None):
    # Valinnat:
    if prefix in state.keys() and state[prefix] == ot_tyyppi[1:]:
        saveField(state, data, "parhaan_haku", ['parametrit'],
                  'parhaan_haku', 'arvo', prefix + ot_tyyppi)
    ...
    if state and prefix in state.keys() and state[prefix] == ot_tyyppi[1:]:
        if state[prefix + ot_tyyppi + "_parhaan_haku"] == "":
            if not validate(state, "kiintea", validiointi[0],
                            prefix + ot_tyyppi):
                data['valid'] = False
                errors = validiointi[1]
            else:
                saveField(state, data, "kiintea", ['parametrit'],
                          'parhaan_kaava', 'arvo', prefix + ot_tyyppi, muunnos)

    try:
        if not state[prefix + ot_tyyppi + "_parhaan_haku"] == "":
            save_data(data, ['parametrit'], 'parhaan_kaava',
                      'arvo', "suor*muk")
    except Exception:
        pass
    ...
    if state and prefix in state.keys() and state[prefix] == ot_tyyppi[1:]:
        if not validate(state, "jaettavat", [is_number], prefix + ot_tyyppi):
            ...
            saveField(state, data, "jaettavat", ['parametrit'],
                      'jaettavat', 'arvo', prefix + ot_tyyppi)
    formi.update(field(state, "jaettavat", prefix + ot_tyyppi, errors))


def nollasuoritus(state, data, prefix, ot_tyyppi, formi,
                  validiointi=[[is_number], "Syötä numeroita!"], muunnos=None):
    # Kiintea nolla
    errors = ""
    if state and prefix in state.keys() and state[prefix] == ot_tyyppi[1:]:
        if state[prefix + ot_tyyppi + "_nollan_kerroin"] == "1":
            if not validate(state, "nollan_kaava",
                            validiointi[0], prefix + ot_tyyppi):
                data['valid'] = False
                errors = validiointi[1]
            else:
                saveField(state, data, "nollan_kaava",
    ...
    if state and prefix in state.keys() and state[prefix] == ot_tyyppi[1:]:
        ...
        if kerroin == "1.5" or kerroin == "0.5"or kerroin == "1":
            saveField(state, data, "nollan_kerroin", ['parametrit'],
                      'nollan_kerroin', 'arvo', prefix + ot_tyyppi)
    ...
    errors = ""
    if state and prefix in state.keys() and state[prefix] == ot_tyyppi[1:]:
        if state[prefix + ot_tyyppi + "_nollan_kerroin"] == "m":
            if not validate(state, "muu_kerroin", [is_number],
                            prefix + ot_tyyppi):
                data['valid'] = False
                errors = "Syötä numeroita!"
            else:
                saveField(state, data, "muu_kerroin", ['parametrit'],
                          'nollan_kerroin', 'arvo', prefix + ot_tyyppi)
    formi.update(field(state, "muu_kerroin", prefix + ot_tyyppi, errors))


def arviointi(state, data, prefix, ot_tyyppi, formi,
              validiointi=[[is_number], "Syötä numeroita!"], muunnos=None):
    # Arviointi
    errors = ""
    if state and prefix in state.keys() and state[prefix] == ot_tyyppi[1:]:
        saveField(state, data, "arvio", ['parametrit'],
                  'arvio', 'arvo', prefix + ot_tyyppi)
        if prefix + ot_tyyppi + "_arvio" in state.keys():
            if state[prefix + ot_tyyppi + "_arvio"] == "abs":
                if not validate(state, "oikea", validiointi[0],
                                prefix + ot_tyyppi):
                    errors = validiointi[1]
                    data['valid'] = False
                else:
                    saveField(state, data, "oikea", ['parametrit'],
                              'oikea', 'arvo', prefix + ot_tyyppi, muunnos)
        else:
...
def aikaValiForm(posti, data, prefix):
    ...
    if posti and prefix in posti.keys() and posti[prefix] == "ala":
        ...
        for k, v in data['maaritteet'].items():
            v['tyyppi'] = 'aika'
        save_data(data, ['parametrit'], 'vartion_kaava',
                  'arvo', "aikavali(a,b)")
        data["kaava"] = peruskaava
    # Aloitusarvot kannasta
    state = None
...
def vapaaKaavaForm(posti, data, prefix):
    ...
    if 'maaritteet' in data.keys():  # Määritteet:
        ...
        if posti and prefix + '_maaritteita' in posti.keys():
            maara = int(posti[prefix + '_maaritteita'])
        if maara > int(maara / 5) * 5:
            maara = int(maara / 5) * 5 + 5
        else:
            maara = int(maara / 5) * 5
        ...
        if 'maaritteet' in data.keys():
            ...
            for i in range(maara):
                if maaritteet[i][1]['kali_vihje'] == "":
                    if type(maaritteet[i][0]) == str:
                        if maaritteet[i][0][:1] == "#":
                            del data['maaritteet'][maaritteet[i][0]]
                        else:
                            maarite = maaritteet[i][1]
                            data['maaritteet'][-maaritteet[i][0]] = maarite
                            del data['maaritteet'][maaritteet[i][0]]
        data["kaava"] = peruskaava
    ...
    if state and prefix in state.keys() and state[prefix] == "vk":
        if not validate(state, "kaava", [is_kaava], prefix + "_vk"):
            ...
            data['valid'] = False
        saveField(state, data, "kaava", ['parametrit'],
                  'vartion_kaava', 'arvo', prefix + "_vk")
    formi.update(field(state, "kaava", prefix + "_vk", errors=errors))

    maksimisuoritus(state, data, prefix, "_vk",
                    formi, validiointi=[[is_kaava], "Kaava ei toimi!"])
    nollasuoritus(state, data, prefix, "_vk", formi,
                  validiointi=[[is_kaava], "Kaava ei toimi!"])
    arviointi(state, data, prefix, "_vk",
              formi, validiointi=[[is_kaava], "Kaava ei toimi!"])
...
def puhdasKaavaForm(posti, data, prefix):
    ...
    formi = {"vapaa": True,
             'maaritteet': formit,
             'maaritteita': {'value': maara,
                             'name': prefix + '_maaritteita'}}
...
# Tehtävän määritys #
#####################
def tehtavanMaaritysForm(posti, data, sarja_id,
                         suurin_jarjestysnro=0, prefix="tehtava_", tags={}):
    formidata = []
        ...
        if not k == 'valid':
            ...
            if posti and prefix + str(k) + '_osatehtavia' in posti.keys():
                ...
                if not is_number(posti[prefix + str(k) + '_osatehtavia']):
                    ...
                    osatehtavia = int(posti[prefix + str(k) + '_osatehtavia'])
            elif osatehtavia == 0:
                osatehtavia = 1

            osatehtavia_prefix = prefix + str(k) + '_osatehtavia'
            formidata.append(('osatehtavia', {'id': osatehtavia_prefix,
                                              'name': osatehtavia_prefix,
                                              'value': osatehtavia,
                                              'errors': errors}))
            ...
            osatehtava_id = 0
            osatehtavat = []
            for tk, tv in v['osa_tehtavat'].items():
                osatehtavat.append((tv['nimi'], tk))

            for ot in sorted(osatehtavat, key=operator.itemgetter(0)):
                tk = ot[1]
                tv = v['osa_tehtavat'][tk]
                if osatehtava_id < osatehtavia:
                    ot_formit.append(osaTehtavaForm(posti, tv,
                                                    prefix + str(k) +
                                                    "_" + str(tk)))
                    if 'valid' in tv.keys() and tv['valid'] is False:
                        del tv['valid']
                        data['valid'] = False
                    osatehtava_id += 1
                else:
            ...
            while osatehtava_id < osatehtavia:
                ...
                v['osa_tehtavat'][uusi_id] = {'nimi': letters,
                                              ...
                                              'kaava': "",
                                              'tehtava': tId}
                form_prefix = prefix + str(k) + "_" + str(uusi_id)
                ot_formit.append(osaTehtavaForm(posti,
                                                v['osa_tehtavat'][uusi_id],
                                                form_prefix))
                if 'valid' in v['osa_tehtavat'][uusi_id].keys():
                    if not v['osa_tehtavat'][uusi_id]['valid']:
            ...
            for fk, fv in v.items():
                ...
                if posti and id in posti.keys():
                    ...
                    if fk == 'jarjestysnro':
                        ...
                        if fk == 'nimi':
                            if not is_string(value):
                                ...
                                data['valid'] = False
                            else:  # Korvataan väli alaviivalla.
                        ...
                        if fk == 'lyhenne':
                            if value and not is_unicode_string(value):
                                ...
                                data['valid'] = False
                        if fk == 'rastikasky':
                            # Estetään muiden kuin validien osotteiden syöttö
                    ...
                    if not fk == 'osa_tehtavat':
                        formidata.append((fk, {'id': id,
                                               'name': id,
                                               'value': value,
                                               'errors': errors}))
                    data[k][fk] = value
...
def luoTehtavaData(tehtavat):
    """
    Luo sanakirjan tehtavista, niiden osatehtavista,
    parametreista seka syotteiden maaritteista.
    """
    data = []
    objektit = []
...
def tallennaTehtavaData(data):
    tehtava_id = None
    if 'valid' in data.keys() and data['valid'] is True:
        del data['valid']
        for k, v in data.items():
            # jokainen tehtava:
            teht = v.copy()
            ...
            for ot_k, ot_v in v['osa_tehtavat'].items():
                # jokainen osatehtava:
                osateht = ot_v.copy()
                if 'parametrit' in osateht.keys():
                    del osateht['parametrit']
                if 'maaritteet' in osateht.keys():
                    del osateht['maaritteet']
                ...
                if osa_tehtava.id > 0 or osa_tehtava.id is None:
                    osa_tehtava.save()
                    if 'parametrit' in ot_v.keys():
                        for p_k, p_v in ot_v['parametrit'].items():
                            ...
                            if parametri.id > 0 or parametri.id is None:
                                parametri.save()
                            else:
                                parametri.delete()
                    ...
                    if 'maaritteet' in ot_v.keys():
                        for m_k, m_v in ot_v['maaritteet'].items():
                            ...
                            if maarite.id > 0 or maarite.id is None:
                                maarite.save()
                            else:
                                maarite.delete()
                else:
