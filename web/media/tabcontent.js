...
ddtabcontent.prototype = {
  expandit: function (tabid_or_position) {
    ...
    try {
      if (
        ...
        //if specified tab contains "rel" attr
  ...
  cycleit: function (dir, autorun) {
    ...
    if (typeof autorun == "undefined")
      //if cycleit() is being called by user, versus autorun() function
      this.cancelautorun(); //stop auto cycling of tabs (if running)
  ...
  urlparamselect: function (tabinterfaceid) {
    ...
    ); //check for "?tabinterfaceid=2" in URL
  ...
  expandtab: function (tabref) {
    ...
    for (var i = 0; i < this.tabs.length; i++) {
      ...
      this.getselectedClassTarget(this.tabs[i]).className =
        this.tabs[i].getAttribute("rel") == subcontentid ? "selected" : "";
    }
    if (this.enabletabpersistence)
      //if persistence enabled, save selected tab position(int) relative to its peers
  ...
  expandsubcontent: function (subcontentid) {
    for (var i = 0; i < this.subcontentids.length; i++) {
      ...
      subcontent.style.display =
        subcontent.id == subcontentid ? "block" : "none"; //"show" or hide sub content based on matching id attr value
    }
  },
  ...
  init: function (automodeperiod) {
    ...
    for (var i = 0; i < this.tabs.length; i++) {
      ...
      if (this.tabs[i].getAttribute("rel")) {
        ...
        this.subcontentids[this.subcontentids.length] =
          this.tabs[i].getAttribute("rel"); //store id of sub content ("rel" attr value)
    ...
    else this.expandtab(this.tabs[this.hottabspositions[0]]); //Just select first tab that contains a "rel" attr
    if (
