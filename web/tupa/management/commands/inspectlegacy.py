...
class Command(NoArgsCommand):
    ...
    def handle_inspection(self, options, koodi):
        table2model = (
            lambda table_name: table_name.title()
            .replace("_", "")
            .replace(" ", "")
            .replace("-", "")
        )
        ...
        for table_name in connection.introspection.get_table_list(cursor):
            ...
            # print luokka
            runko = ""
            sisennys_haku = re.search(
                ".*?(?=#gen_legacy_class " + luokka + "\n)", koodi
            )
            sisennys = ""
            if sisennys_haku:
            ...
            for i, row in enumerate(
                ...
                if i in relations:
                    rel_to = (
                        relations[i][1] == table_name
                        and "'self'"
                        or table2model(relations[i][1])
                    )
            ...
            koodi = re.sub(
                r"(?s)(?<=#gen_legacy_class " + luokka + "\n).*?#end_legacy_class",
                runko + "\n" + sisennys + r"#end_legacy_class",
                koodi,
            )
