...
def kipaResponseRedirect(url): return HttpResponse(
...
def tehtavanTilanne(tehtava):
    ...
    syotteita = Syote.objects.filter(
        maarite__osa_tehtava__tehtava=tehtava).exclude(arvo='kesk').count()
    kesk_syotteita = Syote.objects.filter(
        maarite__osa_tehtava__tehtava=tehtava).filter(arvo='kesk').count()
    maaritteita = SyoteMaarite.objects.filter(
        osa_tehtava__tehtava=tehtava).count()
    tila = ("a", tehtava.nimi)
    if syotteita:
        tila = ("o", tehtava.nimi)
...
def maaritaValitseTehtava(request, kisa_nimi):
    ...
    for s in sarjat:
        formsetti = TehtavaLinkkilistaFormset(posti,
                                              queryset=Tehtava.objects.filter(
                                                  sarja=s),
...
def syotaTehtava(request, kisa_nimi, tehtava_id, talletettu=None, tarkistus=None):
    ...
    tehtavan_syotteet = Syote.objects.filter(
        maarite__osa_tehtava__tehtava=tehtava)
    ...
    for v in vartiot:
        ...
        for ot in osatehtavat:
            ...
            for m in ot.syotemaarite_set.all():
                maaritteen_syotteet = tehtavan_syotteet.filter(
                    vartio=v).filter(maarite=m)
                ...
                if tarkistus:  # Syötetäänkö tarkistus syötteitä?
                    formi = TarkistusSyoteForm(
                        m, v, posti, instance=syote, prefix=str(v.nro)+"_"+str(m.pk),)
                else:  # Syötetään normi syötteitä.
                    formi = SyoteForm(m, v, posti, instance=syote,
    ...
    if posti and validi:  # Sivu oikein
        ...
        if tarkistus:
            ...
            return kipaResponseRedirect("/kipa/"+kisa_nimi+"/syota/tehtava/"+str(tehtava.id)+'/talletettu/')
    else:
        tal = ""
        if talletettu == "talletettu" and not posti:
            tal = "Talletettu!"
...
def sarjanTuloksetCSV(request, kisa_nimi, sarja_id):
    ...
    for i in range(len(mukana[1:])):
        vartiorivi = [mukana[i+1][0].tasa, str(numero), unicode(
            mukana[i+1][0].nro), unicode(mukana[i+1][0].nimi), ]
    ...
    for i in range(len(ulkona)):
        vartiorivi = [ulkona[i][0].tasa, str(numero), unicode(
            ulkona[i][0].nro), unicode(ulkona[i][0].nimi), ]
...
def laskennanTilanne(request, kisa_nimi):
    ...
    for s in kisa.sarja_set.all():
        ...
        for t in s.tehtava_set.all():
            ...
            syotteita += Syote.objects.filter(
                maarite__osa_tehtava__tehtava=t).exclude(arvo='kesk').count()
            kesk_syotteita += Syote.objects.filter(
                maarite__osa_tehtava__tehtava=t).filter(arvo='kesk').count()

            if(t.svirhe):
                taulukko[t.jarjestysnro][sarake] = ('v', t.nimi)

        maaritteita = SyoteMaarite.objects.filter(
            osa_tehtava__tehtava__sarja=s).count()
...
def tehtavanVaiheet(request, kisa_nimi, tehtava_id, vartio_id=None):
    ...
    if not len(vartiot):
        ...
        responssi += '<a href="/kipa/lista/maarita/tehtava/' + \
            str(tehtava_id) + '/">' + u'Takaisin määrittelyyn </a> <br><br>'
        responssi += "</body></html>"
    ...
    laskeSarja(tehtava.sarja, tehtavan_syotteet,
               Vartio.objects.filter(id=vartio_id), [tehtava])
    responssi += '<a href="/kipa/lista/maarita/tehtava/' + \
        str(tehtava_id) + '/">' + u'Takaisin määrittelyyn </a> <br><br>'
    for v in vartiot:
        responssi += '<a href="/kipa/'+kisa_nimi+'/maarita/vaiheet/' + \
            str(tehtava_id)+'/'+str(v.id) + '/">' + \
            str(v.nro) + ' ' + v.nimi + '</a> &nbsp; &nbsp;  '
