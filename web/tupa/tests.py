...
class aritmeettinen_laskin_test(unittest.TestCase):
    ...
    def testYhteenlasku(self):
        self.assertEqual(laske('5+5'), Decimal('10'))

    def testYhteenlasku_spacet(self):
        self.assertEqual(laske(' 5 + 5 '), Decimal('10'))

    def testDesimaaliluku(self):
        self.assertEqual(Decimal(laske('0.5*2+10.5000')), Decimal('11.5000'))

    def testMuuttujavirhe(self):
        self.assertEqual(laske('tyhja_muuttuja*5'), 'S')

    def testSulkulauseke(self):
        self.assertEqual(laske('2*(1+9)'), Decimal('20'))

    def testSulut_sisakkain(self):
        self.assertEqual(laske('10*((1+2)+(5*10))'), Decimal('530'))

    def testSulut_vaarinpain(self):
        self.assertEqual(laske('10*)(1+2)+(5*10))'), None)

    def testSulut_vaaramaara(self):
        self.assertEqual(laske('10*(1+2)+(5*10))'), None)

    def testNollallajako(self):
        self.assertEqual(laske('10/0'), None)

    def testTyhjasyote(self):
        self.assertEqual(laske(''), None)

    def testEiaritmtetiikkaa(self):
        self.assertEqual(laske('a+ b-c*d'), "S")

    def testkaksikertomerkkia(self):  # (potenssi)
        self.assertEqual(laske('5+2**5'), Decimal('37'))

    def testkaksiplusmerkkia(self):
        self.assertEqual(laske('5+2++5'), Decimal('12'))

    def testkaksimiinusmerkkia(self):
        self.assertEqual(laske('5+2--5'), Decimal('12'))

    def testplusmiinus(self):
        self.assertEqual(laske('5+2+-5'), Decimal('2'))

    def testkertojako(self):
        self.assertEqual(laske('5+2*/5'), None)

    def testLaskettuNegatiivinenOperandi(self):
        self.assertEqual(laske('3*(1-5)'), Decimal("-12"))

    def testPitkadesimaali(self):
        self.assertNotEqual(
            laske('-0.008333333333333333333333333333*0.0'), None)

    def testPerusmuuttuja(self):
        self.assertEqual(laske('a', {'a': 1}), Decimal("1"))

    def testPerusFunktio(self):
        self.assertEqual(laske('log(a)', {'a': Decimal("100")}), Decimal("2"))

    def testMinimiFunktio(self):
        self.assertEqual(laske('min(a)', {'a': Decimal("4")}), Decimal("4"))

    def testMedFunktio(self):
        self.assertEqual(laske('med(a)', {'a': Decimal("4")}), Decimal("4"))
    # def testMedFunktio2(self):
    #    self.assertEqual( laske('med(a)', {'a': {'a' : Decimal("4"),'b' : Decimal("5"),'c' : Decimal("6")} }) == Decimal("5")
    # Poistettu, koska nykyinen toteutsu ei toimi n�in, muutetaan vastaavaksi joskus...

    def testListaFunktio(self):
        self.assertEqual(
            laske('min([a,3,1])', {'a': Decimal("100")}), Decimal("1"))

    def testAbsMiinusparametri(self):
        self.assertEqual(laske('abs(-1)'), Decimal("1"))

    def testSulkuMiinus(self):
        self.assertEqual(laske('(2)-1'), Decimal("1"))
    ...
    class testi(TestCase):
        ...
        def testSanity(self):
            ...
            for s in sarjat:  # Sarjakohtaiset n�kym�t
                ...
                maaritaTehtava(request, kisa_nimi=kisa_nimi,
                               sarja_id=sarja_id)
                kopioiTehtavia(request, kisa_nimi=kisa_nimi,
                               sarja_id=sarja_id)
                tulostaSarja(request, kisa_nimi=kisa_nimi,
                             sarja_id=sarja_id)
                sarjanTuloksetCSV(
                    request, kisa_nimi=kisa_nimi, sarja_id=sarja_id)
                tulostaSarjaHTML(request, kisa_nimi=kisa_nimi,
                                 sarja_id=sarja_id)
            for t in tehtavat:  # teht�v�kohtaiset n�kym�t
                ...
                syotaTehtava(request, kisa_nimi=kisa_nimi,
                             tehtava_id=tehtava_id)
                tehtavanVaiheet(request, kisa_nimi=kisa_nimi,
                                tehtava_id=tehtava_id)
    return testi
...
def TulosTestFactory(fixture_name):
    ...
    class testi(TestCase):
        ...
        def testTulokset(self):
            ...
            for v in virheet:
                virhe = virhe + "\n--------------------------------\n" + v
            self.assertEqual(len(virheet), 0, unicode(
                virhe).encode('ascii', 'replace'))
            sys.stdout.flush()
        ...
        def testTehtavanUudelleenTallennus(self):
            ...
            for s in Sarja.objects.select_related().all():
                for t in s.tehtava_set.all():
                    ...
                    posti = {}
                    c = Client()
                    osoite = "/kipa/"+s.kisa.nimi + \
                        "/maarita/tehtava/"+str(t.id)+"/"
...
class TasapisteTesti(TestCase):
    ...
    def testJarjestys(self):
        ...
        self.assertEqual(tulokset[0][1][0].nro, 1)
        self.assertEqual(tulokset[0][2][0].nro, 2)
        self.assertEqual(tulokset[0][3][0].nro, 3)
        self.assertEqual(tulokset[0][4][0].nro, 4)
        self.assertEqual(tulokset[0][5][0].nro, 5)
        self.assertEqual(tulokset[0][6][0].nro, 6)
...
class TestRunner(DjangoTestSuiteRunner):
    ...
    def __init__(self, verbosity=1, interactive=True, failfast=True, **kwargs):
        super(TestRunner, self).__init__(
            verbosity, interactive, failfast, **kwargs)
            ...
            # ajettavaksi haluttu yksittäinen fixtuuri
                ...
                for item in range(len(test_fixtures)):
                    if test_fixtures[item].endswith('.xml'):
                        test_fixtures[item] = (
                            '%s/fixtures/tests/%s' % (os.curdir, test_fixtures[item]))
                    else:
                        test_fixtures[item] = (
                            '%s/fixtures/tests/%s.xml' % (os.curdir, test_fixtures[item]))
