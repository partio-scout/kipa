...
def create_kipa_permissions():
    
    # get content type for the app 
    ct, create = ContentType.objects.get_or_create(app_label='tupa', model='kipa')

    # create permissions if it doesn't already exist
    Permission.objects.get_or_create(name="Kisan omistaja",
                                codename='kisa_omistaja',
                                content_type=ct)
    Permission.objects.get_or_create(name="Tuomarineuvosto",
                                codename='kisa_tuomari',
                                content_type=ct)
    Permission.objects.get_or_create(name="Tulostoimiston apina",
                                codename='kisa_jasen',
                                content_type=ct)

def create_kisa_permissions(nimi):
    
    # get content type for the app 
    ct, create = ContentType.objects.get_or_create(app_label='tupa', model='kisa')

    # create a permission if it doesn't already exist
    Permission.objects.get_or_create(name="Kisa: "+nimi,
                                codename='kisa_nimi_'+nimi,
                                content_type=ct)
    
def delete_kisa_permissions(nimi):
    
    print "Delete kisa permission!"
    
    kisa = Permission.objects.get(codename='kisa_nimi_'+nimi)
...
class Kisa(models.Model) :
        ...
        def save(self,*args,**kwargs) : # Tulokset uusiksi tallennuksen yhteydessä
            create_kipa_permissions()
            create_kisa_permissions(self.nimi)
            for i in self.sarja_set.all():
                i.tuloksetUusiksi()
            super(Kisa,self).save(*args,**kwargs)
        def delete(self,*args,**kwargs) : # Tulokset uusiksi tallennuksen yhteydessä
            delete_kisa_permissions(self.nimi)
            super(Kisa,self).delete(*args,**kwargs)
